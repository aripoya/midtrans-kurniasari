...
router.get('/api/admin/dalam-kota-report', verifyToken, (request, env) => {
    request.corsHeaders = corsHeaders(request);
    return getDalamKotaReport(request, env);
});
...
// REMOVED: /api/debug/test-login — leaked password-match results and queried
// real user hashes. Info-disclosure endpoint with no legitimate use.
    ...
    async fetch(request, env, ctx) {
        try {
            ...
            if (pathname === '/api/ai/chat' || pathname === '/api/ai/chat/') {
                timeoutMs = 15000;
            } else if (pathname.includes('/backfill-items')) {
                timeoutMs = 20000;
