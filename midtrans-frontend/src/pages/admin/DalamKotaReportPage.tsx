...
const DalamKotaReportPage: React.FC = () => {
  ...
  const fetchStats = async () => {
    try {
      const response = await adminApi.getDalamKotaStats();
  ...
  const handleExportPDF = async () => {
    try {
      ...
      const loadingToast = toast({
        ...
        description: 'Sedang mengambil semua data pesanan dalam kota',
        status: 'info',
        duration: null,
        isClosable: false,
      });
      ...
      // Title
      doc.setFontSize(18);
      doc.text('Laporan Pesanan Dalam Kota', 14, 20);
      ...
      autoTable(doc, {
        startY: tableStartY,
        head: [['ID', 'Pelanggan', 'Telepon', 'Metode', 'Kurir', 'Total', 'Pembayaran', 'Pengiriman', 'Tanggal']],
      ...
      // Save PDF
      const filename = `Laporan-Dalam-Kota-${new Date().toISOString().split('T')[0]}.pdf`;
      doc.save(filename);
      ...
      <VStack spacing={6} align="stretch">
        ...
        <Box>
          <Heading size="lg" mb={2}>
            Laporan Pesanan Dalam Kota
          </Heading>
          <Text color="gray.600">
            Statistik dan daftar lengkap pesanan pengiriman dalam kota
          </Text>
        </Box>
        ...
        {stats && (
          <>
              ...
              <Card>
                <CardBody>
                  <Stat>
                    ...
                    <StatHelpText>Pesanan dalam kota</StatHelpText>
                  </Stat>
                </CardBody>
              </Card>
            ...
            {stats.by_delivery_method.length > 0 && (
              <Card>
                <CardHeader>
                  <Heading size="md">Breakdown per Metode Pengiriman</Heading>
                </CardHeader>
                <CardBody>
                  <SimpleGrid columns={{ base: 1, md: 3 }} spacing={4}>
                    {stats.by_delivery_method.map((method) => (
                      <Box key={method.method} p={4} borderWidth="1px" borderRadius="md">
                        <Text fontWeight="bold" mb={2}>
                          {method.method}
                        </Text>
                        <Text fontSize="sm" color="gray.600">
                          {method.count} pesanan
                        </Text>
                    ...
                    {[...stats.monthly_trend]
                      ...
                      return (
                        <Box
                          ...
                          onClick={() => navigate(`/admin/dalam-kota-report/weekly/${year}/${monthNum}`)}
                        >
                          <HStack mb={2}>
...
export default DalamKotaReportPage;
