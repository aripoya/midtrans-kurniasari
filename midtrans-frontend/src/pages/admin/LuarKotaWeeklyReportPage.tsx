...
const LuarKotaWeeklyReportPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchWeeklyData = async () => {
      if (!year || !month) {
        ...
        navigate('/admin/luar-kota-report');
        return;
      }

      try {
        setLoading(true);
        const response = await adminApi.getLuarKotaWeeklyBreakdown(parseInt(year), parseInt(month));
  ...
  return (
    <Box maxW="7xl" mx="auto" p={6}>
      <VStack spacing={6} align="stretch">
        <Box>
          <Button leftIcon={<FiArrowLeft />} variant="ghost" mb={4} onClick={() => navigate('/admin/luar-kota-report')}>
            Kembali ke Laporan Bulanan
          </Button>
          <Heading size="lg" mb={2}>
            Laporan Mingguan - {getMonthName(month || '')} {year}
          </Heading>
          <Text color="gray.600">Breakdown pesanan luar kota per minggu</Text>
        </Box>
              ...
              <SimpleGrid columns={{ base: 1, md: 2, lg: 3 }} spacing={4}>
                {weeklyData.map((week, index) => (
                  <Box
                    ...
                    onClick={() =>
                      navigate(
                        `/admin/luar-kota-report/weekly/${year}/${month}/orders?from=${encodeURIComponent(
                          week.week_start
...
export default LuarKotaWeeklyReportPage;
