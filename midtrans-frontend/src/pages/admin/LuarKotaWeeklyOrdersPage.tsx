...
const LuarKotaWeeklyOrdersPage: React.FC = () => {
  ...
  useEffect(() => {
    const run = async () => {
      if (!year || !month || !from || !to) {
        ...
        navigate(`/admin/luar-kota-report/weekly/${year || ''}/${month || ''}`);
        return;
      }
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      const header = `Laporan Mingguan Luar Kota\n${from} s/d ${to}`;
      ...
      for (let idx = 0; idx < detailedOrders.length; idx += 1) {
        ...
        const createdLabel = created && !Number.isNaN(created.getTime()) ? created.toLocaleString('id-ID') : String(o.created_at || '');
        ...
        writeWrapped(`Kurir    : ${(o as any).courier_service || '-'}`, 8, 'normal');
        writeWrapped(`Resi     : ${(o as any).tracking_number || '-'}`, 8, 'normal');
      ...
      const filename = `Laporan-Mingguan-Luar-Kota_${from}_${to}.pdf`;
      doc.save(filename);
    } catch (error) {
      ...
      <VStack spacing={6} align="stretch">
          ...
          <Flex justify="space-between" align="flex-start" gap={4} wrap="wrap">
            <Box>
              <Button
                ...
                variant="ghost"
                mb={4}
                onClick={() => navigate(`/admin/luar-kota-report/weekly/${year}/${month}`)}
              >
              ...
              <Text color="gray.600">{titleRange ? `Periode: ${titleRange}` : 'Periode tidak diketahui'}</Text>
            </Box>
            <Button
              leftIcon={<FiDownload />}
              colorScheme="green"
        ...
        {orders.length === 0 ? (
          ...
          <Box overflowX="auto">
            <Table variant="simple" size="sm">
              ...
              <Tbody>
                {orders.map((o) => (
                  <Tr key={o.id} cursor="pointer" _hover={{ bg: 'gray.50' }} onClick={() => navigate(`/admin/orders/${o.id}`)}>
                    ...
                    <Td>
                      <Badge colorScheme={o.shipping_status === 'diterima' ? 'green' : 'blue'}>
                        {o.shipping_status}
                      </Badge>
                    </Td>
        ...
        {orders.length > 0 && (
          <HStack justify="space-between">
            ...
            <Button variant="outline" onClick={() => navigate('/admin/luar-kota-report')}>
              Ke Laporan Bulanan
            </Button>
          </HStack>
        )}
...
export default LuarKotaWeeklyOrdersPage;
