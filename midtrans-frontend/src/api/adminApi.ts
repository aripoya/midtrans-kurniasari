...
const getAdminToken = (): string | null => {
  ...
  console.log('[DEBUG] Retrieved token from sessionStorage:', token ? 'Token exists' : 'No token found');
  return token;
};
...
export const adminApi = {
  ...
  getOrderDetails: async (orderId: string): Promise<OrderResponse> => {
    try {
      console.log(`🔍 Getting order details for ID: ${orderId} from ${API_BASE_URL}`);
      ...
      return {
        success: false,
        data: null,
        error: error.response?.data?.error || error.message || 'Error saat mengambil detail pesanan'
      };
    }
  },
  
  // Update status pengiriman pesanan
  updateOrderStatus: async (orderId: string, status: string, adminNote: string = ''): Promise<ApiResponse> => {
    try {
  ...
  updateOrderDetails: async (orderId: string, shippingData: UpdateOrderDetailsRequest): Promise<ApiResponse> => {
    try {
  ...
  getAdminOrders: async (offset: number = 0, limit: number = 50): Promise<OrdersResponse> => {
    try {
      ...
      if (response.data?.success && Array.isArray(response.data?.orders)) {
        ...
        console.log('[DEBUG] Response is direct array with length:', orders.length);
        return {
          success: true,
          data: {
            orders,
      ...
      return {
        success: false,
        data: undefined,
        error: error.response?.data?.error || error.message || 'Error saat mengambil daftar pesanan admin'
      };
    }
  },
  ...
  uploadShippingImage: async (orderId: string, imageType: 'siap_kirim' | 'pengiriman' | 'diterima', imageFile: File): Promise<ApiResponse> => {
    try {
      console.log(`📤 Uploading shipping image for order ${orderId}`);
    ...
    // Extract imageUrl from response for frontend display
    const imageUrl = response.data?.imageUrl || response.data?.url || null;
    if (!imageUrl) {
      console.warn('⚠️ Backend did not return imageUrl in response');
    }
    ...
    return { 
      ...
      data: {
        ...response.data,
        imageUrl // Ensure imageUrl is always available for frontend
      }, 
      error: null 
    };
    } catch (error: any) {
      ...
      return {
        success: false,
        data: null,
        error: error.response?.data?.error || error.message || 'Error saat mengupload gambar'
      };
    }
  },

  // Mendapatkan gambar status pengiriman
  getShippingImages: async (orderId: string): Promise<ShippingImagesResponse> => {
    try {
      const response: AxiosResponse = await axios.get(
        `${API_BASE_URL}/api/orders/${orderId}/shipping-images`,
        {
  ...
  deleteShippingImage: async (orderId: string, imageType: 'siap_kirim' | 'pengiriman' | 'diterima'): Promise<ApiResponse> => {
    try {
      const response: AxiosResponse = await axios.delete(
  ...
  // Update status pengiriman pesanan
  updateOrderShippingStatus: async (orderId: string, status: string): Promise<ApiResponse> => {
    try {
      console.log(`🚚 Updating shipping status for order ${orderId} to: ${status}`);
  ...
  getLocations: async (): Promise<LocationsResponse> => {
    try {
      ...
      return {
        success: false,
        data: null,
        error: error.response?.data?.error || error.message || 'Error saat mengambil daftar lokasi'
      };
    }
  },
  ...
  getDeliveryOrders: async (): Promise<OrdersResponse> => {
    try {
      ...
      return {
        success: false,
        data: undefined,
        error: error.response?.data?.error || error.message || 'Error saat mengambil pesanan pengiriman'
      };
    }
  },
  ...
  getUsers: async (): Promise<UsersResponse> => {
    try {
      ...
      return {
        success: false,
        data: null,
        error: error.response?.data?.error || error.message || 'Error saat mengambil daftar user'
      };
    }
  },
  ...
  createUser: async (userData: CreateUserRequest): Promise<ApiResponse<User>> => {
    try {
      ...
      return {
        success: false,
        data: null,
        error: error.response?.data?.error || error.message || 'Error saat membuat user baru'
      };
    }
  },
  ...
  updateUser: async (userId: string, userData: UpdateUserRequest): Promise<ApiResponse<User>> => {
    try {
      ...
      return {
        success: false,
        data: null,
        error: error.response?.data?.error || error.message || 'Error saat mengupdate user'
      };
    }
  },
  ...
  deleteUser: async (userId: string): Promise<ApiResponse> => {
    try {
      ...
      return {
        success: false,
        data: null,
        error: error.response?.data?.error || error.message || 'Error saat menghapus user'
      };
    }
  },
  ...
  resetPassword: async (userId: string, newPassword: string): Promise<ApiResponse> => {
    try {
      ...
      return {
        success: false,
        data: null,
        error: error.response?.data?.error || error.message || 'Error saat reset password user'
      };
    }
  }
};
